...
<nav className="sidebar-nav">
    ...
    <div className="nav-group" key={g.title}>
      {g.items.some(([key]) => menuPermissionForRole(key, userRole, dbPerms)) && sidebar && (
        <button className="group-title" onClick={() => setOpen(v => ({ ...v, [g.title]: !v[g.title] }))}>
          <span>{g.title}</span>
          <span><AdminIcon name={open[g.title] ? 'chevronDown' : 'chevronRight'}/></span>
        </button>
      )}
      {(sidebar ? open[g.title] : true) && g.items.filter(([key]) => menuPermissionForRole(key, userRole, dbPerms)).map(([key, label, icon]) => (
        <button 
          ...
          title={label}
        >
          <span className="nav-icon"><AdminIcon name={icon}/></span>
  ...
  <main className="talenta-main"><header className="topbar"><button className="icon-btn" aria-label="Buka menu" onClick={()=>setSidebar(v=>!v)}><Icon name="menu"/></button><div className="crumb"><span>MoonXprojecT</span><b>/</b>{activeLabel}</div><div className="top-actions"><div className="search-global"><span><Icon name="search"/></span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cari data..."/></div><button className="icon-btn" aria-label="Muat ulang" onClick={()=>refresh()}><Icon name="refresh"/></button><div className="avatar">HR</div></div></header>
   <section className="page">{loading&&<div className="loading">Memuat data…</div>}{error&&<div className="alert">{error}</div>}
    ...
    {menu==='settings'&&<Settings/>}{menu==='roles'&&<RoleEditorEnterprise userRole={userRole}/>} {menu==='audit'&&<Audit/>}{menu==='approvals'&&<ApprovalCenter/>}{menu==='notifications'&&<Notifications/>} {menu==='system-health'&&<SystemHealth/>}{menu==='enterprise-v26'&&<EnterpriseV20 employees={employees}/>}{menu==='security-v21'&&<SecurityCenterV21/>} 
...
function Stat({title,value,hint,icon}:{title:string;value:string;hint:string;icon:string}){return <div className="stat-card"><div className="stat-icon"><AdminIcon name={icon}/></div><div><span>{title}</span><strong>{value}</strong><small>{hint}</small></div></div>}
function Quick({label,icon,onClick}:{label:string;icon:string;onClick:()=>void}){return <button className="quick-action" onClick={onClick}><span className="quick-icon"><AdminIcon name={icon}/></span>{label}<span aria-hidden="true">›</span></button>}
...
function LeaveModule({initial}:{initial:MenuKey}){
 ...
 const items=[['inbox','Approval Inbox','inbox'],['requests','Pengajuan Baru','plus'],['history','Riwayat','calendar'],['balance','Saldo Cuti','balance']].map(([key,label,icon])=>({key,label,icon}));
